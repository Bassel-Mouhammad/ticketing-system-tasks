                    ...
                    <div class="card mb-3 shadow-sm">
                        <div class="card-body">
                            ...
                            <p class="card-text"><strong>Status:</strong>
                                <span
                                    class="badge bg-success">{{ ucfirst(optional($ticket->status)->name) ?? 'Unknown' }}</span>
                            </p>

                            <!-- Ensure user is not null -->
                            <p class="card-text"><strong>Created By:</strong>
                                {{ optional($ticket->user)->name ?? 'Unknown' }}</p>
                            ...
                            <p class="card-text"><strong>Created At:</strong>
                                {{ $ticket->created_at ? $ticket->created_at->format('Y-m-d H:i') : 'Not available' }}</p>
                            <p class="card-text"><strong>Updated At:</strong>
                                {{ $ticket->updated_at ? $ticket->updated_at->format('Y-m-d H:i') : 'Not available' }}</p>
                        </div>
                    </div>
                @endforeach
            @endif
                ...
                @foreach ($tickets->where('status.name', 'ongoing') as $ticket)
                    <div class="card mb-3 shadow-sm">
                        <div class="card-body">
                            ...
                            <p class="card-text"><strong>Status:</strong>
                                <span
                                    class="badge bg-warning">{{ ucfirst(optional($ticket->status)->name) ?? 'Unknown' }}</span>
                            </p>
                ...
                @foreach ($tickets->where('status.name', 'finished') as $ticket)
                    <div class="card mb-3 shadow-sm">
                        <div class="card-body">
                            ...
                            <p class="card-text"><strong>Status:</strong>
                                <span
                                    class="badge bg-secondary">{{ ucfirst(optional($ticket->status)->name) ?? 'Unknown' }}</span>
                            </p>
